...
export default function UserPage({ provider, signer, address, saleAddress, gnrAddress, usdtAddress }) {
        ...
        try {
            ...
            if (autoStakeAfterBuy) {
                ...
                try {
                    ...
                    hideStake();
                    message.success(urlStake ? _jsx("a", { href: urlStake, target: "_blank", rel: "noreferrer", children: "\u5DF2\u81EA\u52A8\u8D28\u62BC\uFF0C\u67E5\u770B\u4EA4\u6613" }) : "已自动质押");
                    await reload();
                }
                catch (se) {
                    hideStake();
    ...
    async function onFaucetUSDT() {
        ...
        try {
            ...
            if (balNative === 0n) {
                message.error("当前网络原生币余额不足（需 tBNB 用于 Gas）");
                return;
            }
            ...
            if (msg.includes("Internal JSON-RPC error") || msg.includes("UNKNOWN_ERROR")) {
                message.error("交易提交失败：请确认 BSC 测试网账户有足够 tBNB 支付 Gas");
                return;
            }
